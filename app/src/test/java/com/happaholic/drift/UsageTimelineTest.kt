...
class UsageTimelineTest {
    ...
    fun `verified focus removes only overlapping Drift foreground time`() {
        val result = calculateAdjustedForegroundDurations(
            events = listOf(
                event(0, "com.happaholic.drift", UsageTimelineEventType.Resumed),
                event(100, "com.happaholic.drift", UsageTimelineEventType.Paused)
            ),
            rangeStart = 0,
            rangeEnd = 100,
            driftPackageName = "com.happaholic.drift",
            verifiedFocusIntervals = listOf(FocusSessionInterval(20, 80))
        )

        assertEquals(40L, result["com.happaholic.drift"])
    }
